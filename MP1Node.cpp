...
bool MP1Node::recvCallBack(void *env, char *data, int size ) {
    ...
    if(receivedMsg->msgType == JOINREQ) {
        ...
        //make new membership entry and add membership list to the node
        ...
        if(!existsNodeInMemberListTable(id)) {
            //make new membership entry and add to the membership list of the node
            addNodeToMemberListTable(id, port, heartbeat, memberNode->timeOutCounter);
        } else {
...
void MP1Node::nodeLoopOps() {
	 ...
	 * Your code goes here
	 */
    /*1.if the heatbeat has not increased for more than TFAIL seconds, the member is considered
     *failed
     *2.after TREMOVE Seconds, it will delete the member from the list
     */

    //check if node should sent a new heartbeat
